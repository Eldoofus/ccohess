...
struct move {
    ...
    __attribute__((always_inline)) inline bool operator>(const ::move& m){
        return (ft >> 12) > (m.ft >> 12);
    }

    __attribute__((always_inline)) inline void operator()(mptr m, const ull& from, const ull& to, const int& score = 0){
        move = m;
        ft = ((square(from) & 0b111111) << 6) | (square(to) & 0b111111) | ((score & 0b11111111111111111111) << 12);
    }
};
...
namespace mList {
    ...
    template<class status status>
    __attribute__((always_inline)) inline void init(board& brd, int depth){
    ...
    template<class status status>
    __attribute__((always_inline)) inline void pinD12(ull king, ull enemy, board& brd, ull& epTarget){
    ...
    template<class status status>
    __attribute__((always_inline)) inline void pinHV(ull king, ull enemy, board& brd){
        const ull pinmask = pMap[king * 64 + enemy];
    ...
    template<class status status>
    __attribute__((always_inline)) inline void pinEP(ull kingsquare, ull king, ull enemyRQ, board& brd){
        constexpr bool white = status.wMove;
    ...
    template<class status status>
    __attribute__((always_inline)) inline ull refresh(board& brd, ull& kingban, ull& checkmask, int depth){
        constexpr bool white = status.wMove;
        constexpr bool enemy = !status.wMove;
    ...
    template<class status status>
    long move_ordering_score(int piece_value, ull t, int promotion_piece_value, const board& brd, int capture=0){
        constexpr bool white = status.wMove;
        long score = 0;
        int t_material = brd.getMaterial(t);

        // for captures. 1 means confirm have capture, -1 means confirm no capture
        if (capture != -1 && (capture == 1 || white && t_material<0 || !white && t_material>0)){
            score += (10*abs(t_material) - piece_value);
        }
    ...
    __attribute__((always_inline)) inline void _pawntakes(mptr Pq, mptr Pr, mptr Pb, mptr Pn, mptr Patk, ::move moves[], int& cnt, ull Lpawns, ull Rpawns, const ull& filter){
        ...
        if ((Lpawns | Rpawns) & Pawns_LastRank<white>()){
            ...
            while (Promote_Left)    { const ull pos = popBit(Promote_Left);
                moves[cnt++](Pq, Pawn_AttackRight<!white>(pos), pos);
                moves[cnt++](Pr, Pawn_AttackRight<!white>(pos), pos);
                moves[cnt++](Pb, Pawn_AttackRight<!white>(pos), pos);
                moves[cnt++](Pn, Pawn_AttackRight<!white>(pos), pos);
            }
            while (Promote_Right)   { const ull pos = popBit(Promote_Right);
                moves[cnt++](Pq, Pawn_AttackLeft<!white>(pos), pos);
                moves[cnt++](Pr, Pawn_AttackLeft<!white>(pos), pos);
                moves[cnt++](Pb, Pawn_AttackLeft<!white>(pos), pos);
                moves[cnt++](Pn, Pawn_AttackLeft<!white>(pos), pos);
            }
        ...
        while (NoPromote_Left)  { const ull pos = popBit(NoPromote_Left);
            moves[cnt++](Patk, Pawn_AttackRight<!white>(pos), pos);
        }
        while (NoPromote_Right) { const ull pos = popBit(NoPromote_Right);
            moves[cnt++](Patk, Pawn_AttackLeft<!white>(pos), pos);
        }
    }
    __attribute__((always_inline)) inline void _knightmoves(mptr P, ::move moves[], int& cnt, ull knights, const ull& filter){
        forBits(knights) {
            ...
            while (m) moves[cnt++](P, 1ull << sq, popBit(m));
        }
    }
    ...
    __attribute__((always_inline)) inline void _kingmoves(mptr P, ::move moves[], int& cnt, const ull& king, ull kingatk, const ull& filter){
        kingatk &= filter;
        while (kingatk) moves[cnt++](P, king, popBit(kingatk));
    }

    template<class status status, class C, template<class ::status, class> class F>
    __attribute__((always_inline)) inline int _movegen(const board& brd, ull& kingatk, const ull& kingban, const ull& checkmask, ::move moves[], int depth){
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Queenfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Queenfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Queenfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, queensD12, 0, Queenfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, queensHV, 0, Queenfilter);
        _queenmoves(C::template Queenmove<status, F>, brd, moves, cnt, queens2, Queenfilter);
        _kingmoves(C::template Kingmove<status, F>, moves, cnt, king, kingatk, Queenfilter);
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Rookfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Rookfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Rookfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, queensD12, 0, Rookfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, queensHV, 0, Rookfilter);
        _queenmoves(C::template Queenmove<status, F>, brd, moves, cnt, queens2, Rookfilter);
        _kingmoves(C::template Kingmove<status, F>, moves, cnt, king, kingatk, Rookfilter);
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Bishopfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Bishopfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Bishopfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, queensD12, 0, Bishopfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, queensHV, 0, Bishopfilter);
        _queenmoves(C::template Queenmove<status, F>, brd, moves, cnt, queens2, Bishopfilter);
        _kingmoves(C::template Kingmove<status, F>, moves, cnt, king, kingatk, Bishopfilter);
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Knightfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Knightfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Knightfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, queensD12, 0, Knightfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, queensHV, 0, Knightfilter);
        _queenmoves(C::template Queenmove<status, F>, brd, moves, cnt, queens2, Knightfilter);
        _kingmoves(C::template Kingmove<status, F>, moves, cnt, king, kingatk, Knightfilter);
        ...
        if constexpr (status.hasEP) {
            ...
            if (epLpawn | epRpawn) {
                ...
                if (epLpawn) moves[cnt++](C::template PawnEnpassantTake<status, F>, epLpawn, Pawn_AttackLeft<white>(epLpawn));
                if (epRpawn) moves[cnt++](C::template PawnEnpassantTake<status, F>, epRpawn, Pawn_AttackRight<white>(epRpawn));
            }
        }
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Pawnfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Pawnfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Pawnfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, queensD12, 0, Pawnfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, queensHV, 0, Pawnfilter);
        _queenmoves(C::template Queenmove<status, F>, brd, moves, cnt, queens2, Pawnfilter);
        _kingmoves(C::template Kingmove<status, F>, moves, cnt, king, kingatk, Pawnfilter);

        if constexpr (status.canCastleLeft()) if (noCheck && status.canCastleLeft(kingban, brd.occupied, Rooks<white>(brd)))
            moves[cnt++](C::template KingCastle<status, F, status.rookswitchL()>, King<white>(brd), King<white>(brd) >> 2);
        if constexpr (status.canCastleRight()) if (noCheck && status.canCastleRight(kingban, brd.occupied, Rooks<white>(brd)))
            moves[cnt++](C::template KingCastle<status, F, status.rookswitchR()>, King<white>(brd), King<white>(brd) << 2);
        if (Fpawns & Pawns_LastRank<white>()) {
            ...
            while (Promote_Move)    { const ull pos = popBit(Promote_Move);
                moves[cnt++](C::template Pawnpromote<status, F, boardPiece::queen>, pos, Pawn_Forward<white>(pos));
                moves[cnt++](C::template Pawnpromote<status, F, boardPiece::rook>, pos, Pawn_Forward<white>(pos));
                moves[cnt++](C::template Pawnpromote<status, F, boardPiece::bishop>, pos, Pawn_Forward<white>(pos));
                moves[cnt++](C::template Pawnpromote<status, F, boardPiece::knight>, pos, Pawn_Forward<white>(pos));
            }
            while (NoPromote_Move)  { const ull pos = popBit(NoPromote_Move);
                moves[cnt++](C::template Pawnmove<status, F>, pos, Pawn_Forward<white>(pos));
            }
            while (Ppawns)          { const ull pos = popBit(Ppawns); 
                moves[cnt++](C::template Pawnpush<status, F>, pos, Pawn_Forward2<white>(pos));
            }
        } else {
        ...
        _knightmoves(C::template Knightmove<status, F>, moves, cnt, knights, Emptyfilter);
        _bishopmoves(C::template Bishopmove<status, F>, pinD12, brd, moves, cnt, bish_pinned, bish_nopin, Emptyfilter);
        _rookmoves(C::template Rookmove<status, F>, pinHV, brd, moves, cnt, rook_pinned, rook_nopin, Emptyfilter);
    ...
    __attribute__((always_inline)) inline long long _quiesce(const board& brd, ull kingatk, const ull kingban, const ull checkmask, int depth){
        if (depth <= 0) {
            ...
            best = eval;
            ::move moves[218], bestmove;

            C::alpha[depth] = max(eval, C::alpha[depth]);
            if (best >= C::beta[depth]) return best;

            int cnt = _movegen<status, C, QuiesceMoves>(brd, kingatk, kingban, checkmask, moves, depth);
        ...
        __attribute__((noinline)) static long long e(board& brd, int depth){
            ...
            if (checkmask != 0) return _quiesce<status, C>(brd, kingatk, kingban, checkmask, depth);
                ...
                if (depth == 0) {
                    ...
                    forBits (kingatk) {
                        const ull sq = square(kingatk);
                        mGen::enking[depth - 1] = Lookup::King(sq);
                        eval = -C::template Kingmove<status, QuiesceMoves>(brd, King<status.wMove>(brd), 1ull << sq, depth);
                        if(eval > best){
                            best = eval;
                            C::alpha[depth] = max(eval, C::alpha[depth]);
                            bestmove(C::template Kingmove<status, QuiesceMoves>, King<status.wMove>(brd), 1ull << sq, depth);
                        }
                        if(eval >= C::beta[depth]){
                            C::cuts++;
                            break;
    ...
    __attribute__((always_inline)) inline long long _enumerate(const board& brd, ull kingatk, const ull kingban, const ull checkmask, int depth){
        if (depth == 25) {
            board b = brd;
            return QuiesceMoves<status, C>::e(b, 25);
        } else {
            ...
            if(C::fullmove - C::halfmove > 50) return 0;
            ...
            for(int i=0;i<cnt;i++){
                eval = -moves[i](brd, depth);
                if (depth == C::maxdepth) {
                    if (moves[i].move == C::template Pawnpromote<status, EnumerateMoves, boardPiece::queen>) addMove(moves[i].from(), moves[i].to(), 'q');
                    else if (moves[i].move == C::template Pawnpromote<status, EnumerateMoves, boardPiece::rook>) addMove(moves[i].from(), moves[i].to(), 'r');
                    else if (moves[i].move == C::template Pawnpromote<status, EnumerateMoves, boardPiece::bishop>) addMove(moves[i].from(), moves[i].to(), 'b');
                    else if (moves[i].move == C::template Pawnpromote<status, EnumerateMoves, boardPiece::knight>) addMove(moves[i].from(), moves[i].to(), 'n');
                    else addMove(moves[i].from(), moves[i].to());
                }
                if(eval > best){
                    best = eval;
                    C::alpha[depth] = max(eval, C::alpha[depth]);
            ...
            if (checkmask != 0) return _enumerate<status, C>(brd, kingatk, kingban, checkmask, depth);
                ...
                if (depth == 25) {
                    ...
                    forBits (kingatk) {
                        const ull sq = square(kingatk);
                        mGen::enking[depth - 1] = Lookup::King(sq);
                        if (depth == C::maxdepth) addMove(King<status.wMove>(brd), 1ull << sq);
                        eval = -C::template Kingmove<status, EnumerateMoves>(brd, King<status.wMove>(brd), 1ull << sq, depth);

                        if(eval > best){
                            best = eval;
                            C::alpha[depth] = max(eval, C::alpha[depth]);
                            bestmove(C::template Kingmove<status, EnumerateMoves>, King<status.wMove>(brd), 1ull << sq, depth);
                        }
                        if(eval >= C::beta[depth]){
                            C::cuts++;
                            break;
